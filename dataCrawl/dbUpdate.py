...
def showUpdate(datas):
    ...
    # 加入新電影
        ...
        try:
            ...
            show_data=showTimeInfo(movie=movieId,theater=theaterId,date=date,time=time,site=site)
...
def UpdateShows():
    ...
    shows_unique = shows[~shows[['電影名稱','影城','日期']].apply(tuple, axis=1).isin(old_datas.apply(tuple, axis=1))]
    print(shows_unique)
