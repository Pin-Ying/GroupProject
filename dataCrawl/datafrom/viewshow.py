    ...
    for m1 in theaterMode:   #從電影放映版本抓影城
        ...
        for aa in range(len(theaterMode[m1])):
            ...
            for a1 in theaterTime[theaterName]:
                keys = set(a1.keys())
                for i in keys:
                    movieDay=i.replace(' 年 ','-').replace(' 月 ','-').split(' 日')[0] #日期
                    movieTime=a1[i] #時間
...
def pg(url):           
    ...
    for i in s1.find_all("li"):#抓電影網址
        movieName1[i.find("h2").text]="https://www.vscinemas.com.tw/vsweb/film/"+i.find("h2").a["href"]
...
def get_datas():
    ...
    for i in s1.find_all("a"):
        if i.get("href"):
            page.append("https://www.vscinemas.com.tw/vsweb/film/index.aspx"+i.get("href"))
    for pa in page:#頁數
        pg(pa)
    for i in movieName1:
        movie(movieName1[i],i)
    return pd.DataFrame(data,columns=["電影名稱","廳位席位","影城","日期","時間"])
